...
public class PlayerMP : KinematicBody
{
	...
	protected PackedScene BulletScene2 = null;
	protected PackedScene BulletScene3 = null;
	...
	public PackedScene prebullet2;
	public PackedScene prebullet3;
	...
	public override void _Ready()
	{
		...
		prebullet2 = GetSecondBulletScene();
		prebullet3 = GetThirdBulletScene();
		...
		if (IsLocalPlayer)
		{
			...
			NetworkedPlayerSettings.PlayerString = this.Name;//$"Some string {rnd.RandiRange(0, 10000)}";
	...
	{
		return NetworkedPlayerSettings;
	}
	...
	protected void OnShoot(Vector3 Target)
	{
		if (Target != Vector3.Zero)
		{
			...
			//BulletKinematic bullet = (BulletKinematic)prebullet.Instance();
			//Bullet bullet = (Bullet)prebullet.Instance();
			...
			//(bullet as KinematicBody).MoveAndCollide( -(bullet as Spatial).GlobalTransform.basis.z * bullet.speed, true);
			//(bullet as KinematicBody).MoveAndSlide(-(bullet as Spatial).GlobalTransform.basis.z * bullet.speed, Vector3.Up);
		}
	}

	[Remote]
	protected void OnShootRigidBody(Vector3 Target)
	{
		if (Target != Vector3.Zero)
		{
			Bullet bullet = (Bullet)prebullet2.Instance();
	...
	[Remote]
	protected void OnShootKinematicBody(Vector3 Target)
	{
		if (Target != Vector3.Zero)
		{
			BulletKinematic bullet = (BulletKinematic)prebullet3.Instance();
			...
			GetParent().AddChild(bullet);
			bullet.SetTarget(Target);
			//(bullet as KinematicBody).MoveAndCollide( -(bullet as Spatial).GlobalTransform.basis.z * bullet.speed);
			//(bullet as KinematicBody).MoveAndSlide(-(bullet as Spatial).GlobalTransform.basis.z * bullet.speed, Vector3.Up);
		}
	}
	...
	private PackedScene GetBulletScene()
	{
		if (BulletScene == null)
		{
			...
			BulletScene = (PackedScene)ResourceLoader.Load("res://Prefabs/Bullets/bulletOriginalCS.tscn");
			//BulletScene = (PackedScene)ResourceLoader.Load("res://Prefabs/Bullets/bulletOriginalKinematicCS.tscn");
		}

		return BulletScene;
	}

	private PackedScene GetSecondBulletScene()
	{
		if (BulletScene2 == null)
		{			BulletScene2 = (PackedScene)ResourceLoader.Load("res://Prefabs/Bullets/Bullet.tscn");		}
		return BulletScene2;
	}

	private PackedScene GetThirdBulletScene()
	{
		if (BulletScene3 == null)
		{			BulletScene3 = (PackedScene)ResourceLoader.Load("res://Prefabs/Bullets/bulletOriginalKinematicCS.tscn");		}
		return BulletScene3;
	}
	...
	public override void _PhysicsProcess(float delta)
	{
		...
		if (IsLocalPlayer)
		{
			...
			else if (Input.IsMouseButtonPressed(2) && WeaponActiveCooldown <= 0f)//&& RsetActiveCooldown <= 0f)
			{
				/*
				RandomNumberGenerator rnd = new RandomNumberGenerator();
				rnd.Randomize();
				...
				*/
				...
				var from = cammy.ProjectRayOrigin(mouseposition);
				var to = from + cammy.ProjectRayNormal(mouseposition) * 2000f;
				this.MDRpc(nameof(OnShootRigidBody), to);
				OnShootRigidBody(to);
				NetworkedPlayerSettings.PlayerShotCounter++;
				WeaponActiveCooldown = WeaponCooldown;
			}
			else if (Input.IsMouseButtonPressed(8) && WeaponActiveCooldown <= 0f)//&& RsetActiveCooldown <= 0f)
			{
				var mouseposition = GetViewport().GetMousePosition();
				var from = cammy.ProjectRayOrigin(mouseposition);
				var to = from + cammy.ProjectRayNormal(mouseposition) * 2000f;
				this.MDRpc(nameof(OnShootKinematicBody), to);
				OnShootKinematicBody(to);
				NetworkedPlayerSettings.PlayerShotCounter++;
				WeaponActiveCooldown = WeaponCooldown;
			}
			else if (Input.IsMouseButtonPressed(9) && RsetActiveCooldown <= 0f)//&& RsetActiveCooldown <= 0f)
			{
				RandomNumberGenerator rnd = new RandomNumberGenerator();
				rnd.Randomize();
				string someRandom = rnd.RandiRange(0, 100000).ToString();
				Godot.GD.Print(someRandom);
				this.MDRset(nameof(RsetTest), someRandom);
				RsetActiveCooldown = 0.1f;
			}
	...
	public override void _Input(InputEvent @event)
	{
		...
		//if (@event is InputEventMouseButton eventMouseButton) { GD.Print("Mouse Click/Unclick at: ", eventMouseButton.Position); }
		...
		if (@event.IsActionPressed("boost") && BoostCoolDown > 0)
		{			IsBoosting = false;		}
		else if(@event.IsActionReleased("boost") && ((!(BoostCoolDown > 0.05)) || (!(BoostCoolDown < 0.05)))) //&& BoostCoolDown == 0f)
		{
			IsBoosting = false;
			BoostCoolDown = 0.5f;
		}
	...
	Vector3 CalculateVelocity(Vector3 velocityCurrent, Vector3 moveDirection, float delta)
	{
		...
		if (IsBoosting)
		{
			if (BoostLimit < boostLimitAmount && ((!(BoostCoolDown > 0.05)) || (!(BoostCoolDown < 0.05))))//BoostCoolDown == 0)
